...
{       
	...
	lteHelper1->SetEnbAntennaModelType ("ns3::IsotropicAntennaModel");
	...
	lteHelper2->SetEnbAntennaModelType ("ns3::IsotropicAntennaModel");
	lteHelper2->SetEpcHelper (epcHelper2);

	Ptr<LteHelper> lteHelper3 = CreateObject<LteHelper> ();
	Ptr<PointToPointEpcHelper>  epcHelper3 = CreateObject<PointToPointEpcHelper> ();
	lteHelper3->SetEnbAntennaModelType ("ns3::IsotropicAntennaModel");
	lteHelper3->SetEpcHelper (epcHelper3);

	Ptr<LteHelper> lteHelper4 = CreateObject<LteHelper> ();
	Ptr<PointToPointEpcHelper>  epcHelper4 = CreateObject<PointToPointEpcHelper> ();
	lteHelper4->SetEnbAntennaModelType ("ns3::IsotropicAntennaModel");
	lteHelper4->SetEpcHelper (epcHelper4);

	Ptr<LteHelper> lteHelper5 = CreateObject<LteHelper> ();
	Ptr<PointToPointEpcHelper>  epcHelper5 = CreateObject<PointToPointEpcHelper> ();
	lteHelper5->SetEnbAntennaModelType ("ns3::IsotropicAntennaModel");
	lteHelper5->SetEpcHelper (epcHelper5);

	Ptr<LteHelper> lteHelper6 = CreateObject<LteHelper> ();
	Ptr<PointToPointEpcHelper>  epcHelper6 = CreateObject<PointToPointEpcHelper> ();
	lteHelper6->SetEnbAntennaModelType ("ns3::IsotropicAntennaModel");
	lteHelper6->SetEpcHelper (epcHelper6);

	Ptr<LteHelper> lteHelper7 = CreateObject<LteHelper> ();
	Ptr<PointToPointEpcHelper>  epcHelper7 = CreateObject<PointToPointEpcHelper> ();
	lteHelper7->SetEnbAntennaModelType ("ns3::IsotropicAntennaModel");
	lteHelper7->SetEpcHelper (epcHelper7);
...
/*	Ptr<ListPositionAllocator> positionAlloc = CreateObject<ListPositionAllocator> ();
	...
	// HtWifiMacHelper mac = HtWifiMacHelper::Default ();
        ...
        lteHelper4->SetSchedulerType("ns3::PfFfMacScheduler");
        lteHelper5->SetSchedulerType("ns3::PfFfMacScheduler");
        lteHelper6->SetSchedulerType("ns3::PfFfMacScheduler");
        lteHelper7->SetSchedulerType("ns3::PfFfMacScheduler");

	NetDeviceContainer enbLteDevs ;
        Config::SetDefault("ns3::LteEnbPhy::TxPower",DoubleValue(23));
        enbLteDevs.Add(lteHelper2->InstallEnbDevice (enbNodes.Get(1)));
        enbLteDevs.Add(lteHelper4->InstallEnbDevice (enbNodes.Get(3)));
        enbLteDevs.Add(lteHelper6->InstallEnbDevice (enbNodes.Get(5)));
        Config::SetDefault("ns3::LteEnbPhy::TxPower",DoubleValue(15));
        enbLteDevs.Add(lteHelper1->InstallEnbDevice (enbNodes.Get(0)));
        enbLteDevs.Add(lteHelper3->InstallEnbDevice (enbNodes.Get(2)));
        enbLteDevs.Add(lteHelper5->InstallEnbDevice (enbNodes.Get(4)));
        enbLteDevs.Add(lteHelper7->InstallEnbDevice (enbNodes.Get(6)));
	...
	NetDeviceContainer ueLteDevs;
        ...
        NetDeviceContainer ueLteDevs3;
        NetDeviceContainer ueLteDevs4;
        NetDeviceContainer ueLteDevs5;
        NetDeviceContainer ueLteDevs6;
        NetDeviceContainer ueLteDevs7;
	...
	for(uint16_t i=0;i<numberOfClients;i++) {
		//ueLteDevs.Add(lteHelper->InstallUeDevice (integrated_ue.Get(i)));
                if (i>=0 && i<10) 
		{
			ueLteDevs1.Add(lteHelper1->InstallUeDevice (integrated_ue.Get(i)));
		}
		if (i>=10 && i<20)
		{
			ueLteDevs2.Add(lteHelper2->InstallUeDevice (integrated_ue.Get(i)));
		}
		if (i>=20 && i<30)
		{
			ueLteDevs3.Add(lteHelper3->InstallUeDevice (integrated_ue.Get(i)));
		}
		if (i>=30 && i<40)
		{
			ueLteDevs4.Add(lteHelper4->InstallUeDevice (integrated_ue.Get(i)));
		}
		if (i>=40 && i<50)
		{
			ueLteDevs5.Add(lteHelper5->InstallUeDevice (integrated_ue.Get(i)));
		}
		if (i>=50 && i<60)
		{
			ueLteDevs6.Add(lteHelper6->InstallUeDevice (integrated_ue.Get(i)));
		}
		if (i>=60 && i<70)
		{
			ueLteDevs7.Add(lteHelper7->InstallUeDevice (integrated_ue.Get(i)));
		}


	}
        ueLteDevs.Add(ueLteDevs1);
        ueLteDevs.Add(ueLteDevs2);
        ueLteDevs.Add(ueLteDevs3);
        ueLteDevs.Add(ueLteDevs4);
        ueLteDevs.Add(ueLteDevs5);
        ueLteDevs.Add(ueLteDevs6);
        ueLteDevs.Add(ueLteDevs7);
...
for (uint32_t i=0;i<integrated_ue.Get(0)->GetNDevices();i++){
	...
	Ipv4InterfaceContainer ueIpIface7;
        ...
        ueIpIface4 = epcHelper4->AssignUeIpv4Address (NetDeviceContainer (ueLteDevs4));
        ueIpIface5 = epcHelper5->AssignUeIpv4Address (NetDeviceContainer (ueLteDevs5));
        ueIpIface6 = epcHelper6->AssignUeIpv4Address (NetDeviceContainer (ueLteDevs6));
        ueIpIface7 = epcHelper7->AssignUeIpv4Address (NetDeviceContainer (ueLteDevs7));
        

        ueIpIface.Add(ueIpIface1);
        ueIpIface.Add(ueIpIface2);
        ueIpIface.Add(ueIpIface3);
        ueIpIface.Add(ueIpIface4);
        ueIpIface.Add(ueIpIface5);
        ueIpIface.Add(ueIpIface6);
        ueIpIface.Add(ueIpIface7);
	...
	{
		Ptr<Node> ueNode = integrated_ue.Get (i);
		...
		//ueStaticRouting->SetDefaultRoute (epcHelper->GetUeDefaultGatewayAddress (), 1);
                ...
                if (i>=0 && i<10) 
		{
			ueStaticRouting->SetDefaultRoute (epcHelper1->GetUeDefaultGatewayAddress (), 1);

		}
		if (i>=10 && i<20)
		{
			ueStaticRouting->SetDefaultRoute (epcHelper2->GetUeDefaultGatewayAddress (), 1);

		}
		if (i>=20 && i<30)
		{
			ueStaticRouting->SetDefaultRoute (epcHelper3->GetUeDefaultGatewayAddress (), 1);

		}
		if (i>=30 && i<40)
		{
			ueStaticRouting->SetDefaultRoute (epcHelper4->GetUeDefaultGatewayAddress (), 1);

		}
		if (i>=40 && i<50)
		{
			ueStaticRouting->SetDefaultRoute (epcHelper5->GetUeDefaultGatewayAddress (), 1);

		}
		if (i>=50 && i<60)
		{
			ueStaticRouting->SetDefaultRoute (epcHelper6->GetUeDefaultGatewayAddress (), 1);

		}
		if (i>=60 && i<70)
		{
			ueStaticRouting->SetDefaultRoute (epcHelper7->GetUeDefaultGatewayAddress (), 1);
	...
	{
		lteHelper1->Attach (ueLteDevs.Get(ue_num), enbLteDevs.Get(0));
	}
	for(ue_num = 10;ue_num<20;ue_num++) 
	{
		lteHelper2->Attach (ueLteDevs.Get(ue_num), enbLteDevs.Get(1));
	}
	for(ue_num = 20;ue_num<30;ue_num++) 
	{
		lteHelper3->Attach (ueLteDevs.Get(ue_num), enbLteDevs.Get(2));
	}
	for(ue_num = 30;ue_num<40;ue_num++)  
	{
		lteHelper4->Attach (ueLteDevs.Get(ue_num), enbLteDevs.Get(3));
	}
	for(ue_num = 40;ue_num<50;ue_num++) 
	{
		lteHelper5->Attach (ueLteDevs.Get(ue_num), enbLteDevs.Get(4));
	}
	for(ue_num = 50;ue_num<60;ue_num++) 
	{
		lteHelper6->Attach (ueLteDevs.Get(ue_num), enbLteDevs.Get(5));
	}
	for(ue_num = 60;ue_num<70;ue_num++) 
	{
		lteHelper7->Attach (ueLteDevs.Get(ue_num), enbLteDevs.Get(6));
	}
	...
	for (uint16_t i = 0; i < numberOfClients; i++)
	{
		...
		if(i<16)
		{
			...
			//lteHelper->ActivateDedicatedEpsBearer (ueLteDevs.Get(i), bearer, EpcTft::Default ());
                        if (i>=0 && i<10) 
		        {
			        lteHelper1->ActivateDedicatedEpsBearer (ueLteDevs.Get(i), bearer, EpcTft::Default ());
		        }
		        if (i>=10 && i<20)
		        {
		        	lteHelper2->ActivateDedicatedEpsBearer (ueLteDevs.Get(i), bearer, EpcTft::Default ());
		        }
		        if (i>=20 && i<30)
		        {
		        	lteHelper3->ActivateDedicatedEpsBearer (ueLteDevs.Get(i), bearer, EpcTft::Default ());
		        }
		        if (i>=30 && i<40)
		        {
		        	lteHelper4->ActivateDedicatedEpsBearer (ueLteDevs.Get(i), bearer, EpcTft::Default ());
		        }
		        if (i>=40 && i<50)
		        {
		        	lteHelper5->ActivateDedicatedEpsBearer (ueLteDevs.Get(i), bearer, EpcTft::Default ());
		        }
		        if (i>=50 && i<60)
		        {
		               	lteHelper6->ActivateDedicatedEpsBearer (ueLteDevs.Get(i), bearer, EpcTft::Default ());
		        }
		        if (i>=60 && i<70)
		        {
			        lteHelper7->ActivateDedicatedEpsBearer (ueLteDevs.Get(i), bearer, EpcTft::Default ());
		        }
...
//        }
  ...
  clientApps.Stop(Seconds(3.0));*/
	...
	lteHelper4->EnableTraces ();
	lteHelper5->EnableTraces ();
	lteHelper6->EnableTraces ();
	lteHelper7->EnableTraces ();
