    ...
    m_lastUpdateTime (Seconds (0.0))
{
 ...
 //std::cout<<"in liion battery node ="<<GetNode()<<" obj = "<<this<<std::endl;
//ns3::LteHelper::ntolimap[GetNode()]=this;
}
...
LiIonEnergySource::DecreaseRemainingEnergy (double energyJ)
{
  ...
  //std::cout<<Simulator::Now().GetMilliSeconds()<<" " << this << " " << (double)m_remainingEnergyJ << std::endl;
...
LiIonEnergySource::UpdateEnergySource (void)
{
  ...
  Ptr<Node> node=ns3::LteHelper::litonmap[this];

  Ipv4Address    ip=ns3::LteHelper::ntoipmap[node];
  ns3::LteHelper::iptoenergymap[ip]=m_remainingEnergyJ;
 // Ptr<LiIonEnergySource> E=ns3::LteHelper::ntolimap[node];
