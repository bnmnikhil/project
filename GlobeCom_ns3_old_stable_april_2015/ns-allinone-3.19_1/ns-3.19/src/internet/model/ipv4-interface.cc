...
NS_OBJECT_ENSURE_REGISTERED (Ipv4Interface)
				  ;
uint32_t pktno=0;
uint32_t Ipv4Interface::onlylte=1;
TypeId 
...
Ipv4Interface::Send (Ptr<Packet> p, Ipv4Address dest)
{
	...
	if (m_device->NeedsArp ())
	{
		...
		if (dest.IsBroadcast ())
		{
			...
			std::cout<<"Arp  address= "<<hardwareDestination<<std::endl;
			found = true;
		}
		else if (dest.IsMulticast ())
		{
			...
			std::cout<<"Multicast  address= "<<hardwareDestination<<std::endl;
			found = true;
		}
		else
		{
			for (Ipv4InterfaceAddressListCI i = m_ifaddrs.begin (); i != m_ifaddrs.end (); ++i)
			{
				if (dest.IsSubnetDirectedBroadcast ((*i).GetMask ()))
				{
					...
					hardwareDestination = m_device->GetBroadcast ();
					std::cout<<"Broadcast  address= "<<hardwareDestination<<std::endl;
					found = true;
					break;
				}
			}
		...
		if (found)
		{
			...
			//int pkt=rand();
			if(ipv4Header.GetSource().Get()>=117440512 &&ipv4Header.GetSource().Get()<=117440800) {
				//std::cout << desport << std::endl;
				if(Ipv4Interface::onlylte==1 ||Ipv4Interface::onlylte==9 || Ipv4Interface::onlylte==6) {
				...
				else if(Ipv4Interface::onlylte==2) {
					// only wifi
				...
				else if(Ipv4Interface::onlylte==4) {
					std::cout << " onltlte==4      ltewifi in send\n" <<std::endl;
					//lte+wifi
                                        pktno++;
                                       m_device->Send (p, hardwareDestination,Ipv4L3Protocol::PROT_NUMBER);
					...
					/*if(desport%2==0)
					{
						...
						//EpcEnbApplication::uplinkthrpt_lte+=p->GetSize();
						m_device->Send (p, hardwareDestination,Ipv4L3Protocol::PROT_NUMBER);
					}
					else {
						pktno++;
				...
				} else if(Ipv4Interface::onlylte==5) {
			...
			std::cout<<"HW Destination = "<<hardwareDestination<<std::endl;
			m_device->Send (p, hardwareDestination,Ipv4L3Protocol::PROT_NUMBER);
		}
	}
	else
	{
		...
		if(ipv4Header.GetSource().Get()>=117440512 &&ipv4Header.GetSource().Get()<=117440800) {
			//std::cout << "desport " << desport << std::endl;

			if(Ipv4Interface::onlylte==1 || Ipv4Interface::onlylte==9 || Ipv4Interface::onlylte==5) {
				// only lte or UL_lte-DL_wifi
				//	std::cout << "only lte or UL_lte-DL_wifi" << std::endl;
				//std::cout << "only lte Broadcast" <<Simulator::Now()<< std::endl;
				//	std::cout<<"IF1"<<std::endl;

			//	std::cout << seqheader.GetSourcePort() << " " << seqheader.GetDestinationPort() << " " << ipv4Header.GetIdentification() << std::endl;
			...
			else if(Ipv4Interface::onlylte==3) {
				//lte+wifi
				//  std::cout << "lte+wifi" << std::endl;
				//int pkt=rand();
				...
				if(desport%2==0)
				{
					...
					//pktno++;
					//  std::cout << "send through lte interface "<<pktno << "broadcast"<<m_device->GetBroadcast ()<<std::endl;
				//std::cout<<"uplink lte destport no"<<desport<<"source ip"<<ipv4Header.GetSource()<<"destination ip"<<ipv4Header.GetDestination()<<std::endl;
					m_device->Send (p, m_device->GetBroadcast (),Ipv4L3Protocol::PROT_NUMBER);
				}
				else {
					//pktno++;

				//std::cout<< "uplink  wifi "<< desport << std::endl;
					//std::cout << "send through wifi interface " << desport <<"broadcast"<<m_device->GetBroadcast ()<< std::endl;
					RouterLayer router;
					uint32_t i=0;
					for(i=0;i<m_device->GetNode()->GetNDevices();i++) {
						if(m_device->GetNode()->GetDevice(i)->virtualtypeid()=="ns3::WifiNetDevice") {
			...
			else if(Ipv4Interface::onlylte==9) {
				//lte+wifi
				//  std::cout << "lte+wifi" << std::endl;
				//int pkt=rand();
				//std::cout << "ltewifi in sendtoltesocket\n" <<std::endl;
