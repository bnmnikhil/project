     ...
     // pkt->AddHeader(ipv4header);
      ...
      Ipv4FlowProbeTag flowprobe;
       ...
       if(found) {
     	  ...
     	  std::cout<<"Flow ID ="<<fid<<"Packet ID = "<<pid<<std::endl;
     //	  std::cout << "*************receive**********************************\n";
     //	  p->Print(std::cout);
     //	  std::cout << std::endl;
  ...
  for (SocketList::iterator i = m_sockets.begin (); i != m_sockets.end (); ++i)
    {
      ...
      std::cout << "*************Ipv4L3Protocol::Receive  Forwarding to raw socket receive**********************************\n";
      ...
      for (uint32_t j = 0; j < GetNAddresses (ifaceIndex); j++)
        {
          ...
          if (destination.IsSubnetDirectedBroadcast (ifAddr.GetMask ()) && 
              destination.CombineMask (ifAddr.GetMask ()) == ifAddr.GetLocal ().CombineMask (ifAddr.GetMask ())   )
            {
              ...
              packetCopy->AddHeader (ipHeader);
              m_txTrace (packetCopy, m_node->GetObject<Ipv4> (), ifaceIndex);
             // std::cout << outInterface->GetDevice()->GetAddress() << std::endl;
              outInterface->Send (packetCopy, destination);
              return;
            }
        }
...
Ipv4L3Protocol::LocalDeliver (Ptr<const Packet> packet, Ipv4Header const&ip, uint32_t iif)
{
 ...
 /* std::cout<<"**********At Receive LOCAL DELIVER ****************"<<std::endl;
            p->Print(std::cout);
            std::cout<<std::endl;*/
