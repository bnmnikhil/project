...
std::map<Ptr<LteUeMac>,Ptr<Node> > LteHelper::ltonmap;
std::map<Ptr<Node>,Ptr<LiIonEnergySource> > LteHelper::ntolimap;
std::map<Ptr<LiIonEnergySource>,Ptr<Node> > LteHelper::litonmap; 
std::map<Ipv4Address,double > LteHelper::iptoenergymap;
std::map<Ptr<Node>,Ipv4Address > LteHelper::ntoipmap;
...
LteHelper::Attach (Ptr<NetDevice> ueDevice, Ptr<NetDevice> enbDevice)
{
  ...
  ueNas->Connect (enbLteDevice->GetCellId (), enbLteDevice->GetDlEarfcn ());
       std::cout<<ueDevice<<" "<<enbDevice<<std::endl;
  if (m_epcHelper != 0)
    {
      // activate default EPS bearer
