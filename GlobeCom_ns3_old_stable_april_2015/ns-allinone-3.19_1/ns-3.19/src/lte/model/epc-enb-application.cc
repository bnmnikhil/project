...
std::map<Ipv4Address, EpcEnbApplication::sinrperinfo> sinr_table;
std::map<uint32_t, Ipv4Address> rntiipmap;
...
{
        std::cout<<this<<"  epc created --------"<<std::endl;
...
void EpcEnbApplication::udpstats(uint32_t fid,uint32_t pid, double recvbytes, double time) {
	...
	for(std::list<EpcEnbApplication::udp_pktinfo>::iterator it=udp_pktlist.begin();it!=udp_pktlist.end();) {
...
void EpcEnbApplication::updateltesinrper(std::string context, uint16_t cellid,uint16_t rnti, double rsrp, double avgsinr) {
	...
	//	sinrperinfo stemp=sinr_table[ip];
	//	stemp.lteval=avgsinr;
	//	sinr_table[ip]=stemp;
                sinr_table[ip].lteval=avgsinr;
	} else {
		sinrperinfo s;
		s.lteval=avgsinr;
		s.wifival=0;
...
EpcEnbApplication::SendToLteSocket (Ptr<Packet> packet, uint16_t rnti, uint8_t bid)
{
	...
	if(tcp==true) {
		...
		if(found) {
			...
			p.pid=pid;
			EpcEnbApplication::udp_pktlist.push_back(p);
...
uint32_t EpcEnbApplication::topsis(uint32_t gbr, uint32_t currentinterface,uint32_t rank) {
...
uint32_t EpcEnbApplication::topsis1(uint32_t gbr, uint32_t currentinterface,uint32_t rank) {
	////std::cout << "reached here" << std::endl;
...
int EpcEnbApplication::gbr_ensured(uint32_t current_interface) {
	int gbr_ensured=1;
	std::map<uint32_t, flowtable_info>::iterator it;
	for(it=flowtable.begin();it!=flowtable.end();it++) {
		if(it->first!=shiftedflowid) {
...
double EpcEnbApplication::sum(uint32_t current_interface, uint32_t gbr) {
	double sum=0;
	int count=0;
	std::map<uint32_t, flowtable_info>::iterator it;
	for(it=flowtable.begin();it!=flowtable.end();it++) {
...
double EpcEnbApplication::sum1(uint32_t current_interface) {
	double tsum=0;
	//int count=0;
	std::map<uint32_t, flowtable_info>::iterator it;
	for(it=flowtable.begin();it!=flowtable.end();it++) {
		...
		if(it->second.current_interface==current_interface ) {
			tsum+=it->second.throughput;
                       // std::cout<<"flow---------------"<<it->second.throughput<<std::endl;
		}

	}
	return tsum*8/duration/1000/1000;
}
bool EpcEnbApplication::checkNGBR(uint32_t currentinterface) {
	std::map<uint32_t, flowtable_info>::iterator it;
	/*for(it=flowtable.begin();it!=flowtable.end();it++) {
...
void EpcEnbApplication::maximizethrpt() {
...
void EpcEnbApplication::movefairly() {
	double tsum=0;
	double isum=0;
...
int EpcEnbApplication::nflows(uint32_t current_interface) {
	int count=0;
	std::map<uint32_t, flowtable_info>::iterator it;
	for(it=flowtable.begin();it!=flowtable.end();it++) {
		if(it->second.current_interface==current_interface) {
...
void EpcEnbApplication::algoall() {
	int gbr_lte=GBR_lte();
	// no GBR flows
	if(gbr_lte==0) {
		//std::cout << "no gbr " << std::endl;
...
void EpcEnbApplication::algo_naive() {
	double lteload=lte_usedrbs/150;
	double wifiload=wifi_load*100;
//	std::cout << "lteload " << lteload << " wifiload " << wifiload << std::endl;
 ...
 Ipv4Address EpcEnbApplication::lessenergyue(){
std::map<Ipv4Address,double > energy_table=ns3::LteHelper::iptoenergymap;
double leastE=31752;
Ipv4Address ip;
std::map<uint32_t, flowtable_info>::iterator it;
...
Ipv4Address  EpcEnbApplication::moreenergyue(){
std::map<Ipv4Address,double > energy_table=ns3::LteHelper::iptoenergymap;
double moreE=0;
Ipv4Address ip;
std::map<uint32_t, flowtable_info>::iterator it;
...
void EpcEnbApplication::algo_SINR(){
     int flags=0;
...
void EpcEnbApplication::algo_LOAD(){
...
void EpcEnbApplication:: algo_LOAD1(){
//std::cout<<"1 th is called==============================\n";
           double lteload=lte_usedrbs/15000;
	   double wifiload=wifi_load*100/100;
           double canbmovedwifi=(((lteload-wifiload)/2)*(wifitpt/wifiload));
...
int EpcEnbApplication::no_gbrnotensured(uint32_t current_interface) {
	double count=0;
	std::map<uint32_t, flowtable_info>::iterator it;
	for(it=flowtable.begin();it!=flowtable.end();it++) {
		if(it->first!=shiftedflowid) {
...
int EpcEnbApplication::nGflows(uint32_t current_interface, double QOS) {
	int count=0;
	std::map<uint32_t, flowtable_info>::iterator it;
	for(it=flowtable.begin();it!=flowtable.end();it++) {
...
double EpcEnbApplication::nGflowstp(uint32_t current_interface, double QOS) {
	double count=0;
	std::map<uint32_t, flowtable_info>::iterator it;
	for(it=flowtable.begin();it!=flowtable.end();it++) {
		if(it->second.current_interface==current_interface && it->second.Qos_Enabled==QOS) {
...
//std::cout<<"IP =" <<m_sgwS1uAddress<<"m_gtpuUdpPort :" <<m_gtpuUdpPort<<std::endl;
	uint32_t flags = 0;
