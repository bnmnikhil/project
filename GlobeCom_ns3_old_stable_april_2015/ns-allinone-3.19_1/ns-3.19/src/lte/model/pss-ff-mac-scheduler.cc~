...
PssFfMacScheduler::DoSchedDlTriggerReq (const struct FfMacSchedSapProvider::SchedDlTriggerReqParameters& params)
{
	...
	for (itRach = m_rachList.begin (); itRach != m_rachList.end (); itRach++)
	{
		...
		while ((tbSizeBits < (*itRach).m_estimatedSize) && (rbStart + rbLen < m_cschedCellConfig.m_ulBandwidth))
		{
			...
			per_rb_datasize = m_amc->GetTbSizeFromMcs (m_ulGrantMcs, rbLen)/rbLen;
		}
	...
	while (itMap != allocationMap.end ())
	{
		...
		for (uint8_t j = 0; j < nLayer; j++)
		{
			...
			per_rb_datasize= (m_amc->GetTbSizeFromMcs (newDci.m_mcs.at (j),1) / 8);
...
PssFfMacScheduler::DoSchedUlTriggerReq (const struct FfMacSchedSapProvider::SchedUlTriggerReqParameters& params)
{
	...
	std::map <uint16_t,uint32_t>::iterator it;
	std::map <uint16_t,uint32_t>::iterator temp_it;
	int nflows = 0;
	//Assuming 40 flows will be available
	std::map<uint16_t, uint16_t >rs_blocks_per_flow; //Rnti value

	uint32_t total_bsr=0;
	for (it = m_ceBsrRxed.begin (); it != m_ceBsrRxed.end (); it++)
	{
		std::set <uint16_t>::iterator itRnti = rntiAllocated.find ((*it).first);
		if (((*it).second > 0)&&(itRnti == rntiAllocated.end ()))
		{
			total_bsr+=it->second;
			//	std::cout<<"BSR UE" <<it->first <<"value = "<< it->second<<std::endl;
		}

	}
	//std::cout<<"total_bsr = "<<total_bsr<<std::endl;
	uint16_t total_alloctedrb=0;
	for (it = m_ceBsrRxed.begin (); it != m_ceBsrRxed.end (); it++)
	{
		std::set <uint16_t>::iterator itRnti = rntiAllocated.find ((*it).first);
		// select UEs with queues not empty and not yet allocated for HARQ
		...
		{
			nflows++;
			uint16_t temp=(uint16_t)((it->second*(uint32_t(m_cschedCellConfig.m_ulBandwidth))/total_bsr));
			uint16_t flowid =(*it).first;
			rs_blocks_per_flow[flowid]=temp;
			total_alloctedrb+=temp;


		}
	}
	//allocate remaining RB
	...
	for (it = m_ceBsrRxed.begin (); it != m_ceBsrRxed.end (); it++)
	{
		...
		if (((*it).second > 0)&& total_alloctedrb  <=(uint16_t(m_cschedCellConfig.m_ulBandwidth)-1))
		{
			/*if(rs_blocks_per_flow[flowid]<3 )
			{
				if((total_alloctedrb+(3-rs_blocks_per_flow[flowid])) <= ((uint16_t(m_cschedCellConfig.m_ulBandwidth))-1))
				{
					total_alloctedrb+=(3-rs_blocks_per_flow[flowid]);
					rs_blocks_per_flow[flowid]=3;
			...
			total_alloctedrb++;
			rs_blocks_per_flow[flowid]++;
		}
		//std::cout<< "UL bw"<< uint16_t(m_cschedCellConfig.m_ulBandwidth)<< "total allocated RB= "<<total_alloctedrb<<std::endl;
		//	std::cout<<"Rnti ="<< flowid<<"  Expected RB per flow ="<<rs_blocks_per_flow[flowid]<<std::endl;
		last_rnti=flowid;
	}
	...
	//std::cout << (uint32_t)m_cschedCellConfig.m_ulBandwidth << " "<< nflows <<std::endl;
	// Divide the remaining resources equally among the active users starting from the subsequent one served last scheduling trigger
	uint16_t rbPerFlow ;//= (m_cschedCellConfig.m_ulBandwidth) / (nflows + rntiAllocated.size ());
	//std::cout << (uint32_t)m_cschedCellConfig.m_ulBandwidth << " "<< nflows << " " << rbPerFlow <<std::endl;
	// if (rbPerFlow < 3)
	//   {
	//     rbPerFlow = 3;  // at least 3 rbg per flow (till available resource) to ensure TxOpportunity >= 7 bytes
	//   }
	...
	do
	{
		...
		if(firstrnti==(*it).first && flag==false) {
			break;
		} else {
			flag=false;
		}
			//std::cout<< "RNTI"<<rntiAllocated.find ((*it).first)<<std::endl;
                        std::cout<< "RNTI "<<std::endl;
		if ((itRnti != rntiAllocated.end ())||((*it).second == 0)||(rs_blocks_per_flow[(*it).first]==0))
		{
			std::cout<<"************* Required ****** for rnti "<< (*it).first<<" rb= "<<rs_blocks_per_flow[(*it).first]<<std::endl;
			// UE already allocated for UL-HARQ -> skip it
			NS_LOG_DEBUG (this << " UE already allocated in HARQ -> discared, RNTI " << (*it).first);
			//	std::cout<<" UE already allocated in HARQ -> discared, RNTI " << (*it).first<<std::endl;
			//	std::cout<<"First User ="<<(*it).first<<std::endl;



			/*	while(rs_blocks_per_flow[(*it).first]==0){
				if((*it).first!=last_rnti)
							{*/
			...
			//::cout<<"Second User ="<<(*it).first<<std::endl;
			//std::cout<<"Last user"<<m_ceBsrRxed.end ()->first<<std::endl;
			//temp_it=m_ceBsrRxed.begin();
			//std::cout<<"first entry="<<temp_it->first;
			//temp_it=m_ceBsrRxed.end();
			//			// std::cout<<"last entry = "<<last_rnti<<std::endl;
			if((*it).first==last_rnti)
			{
				it = m_ceBsrRxed.begin ();
			} else if((*it).first!=last_rnti)
			{
				it++;
			}
			/*if ((*it).first == m_ceBsrRxed.end ()->first)
			{
		...
		uint16_t newrnti=(*it).first;
		rbPerFlow= rs_blocks_per_flow[newrnti];
		//std::cout<<"************UL BW ="<<(uint16_t)m_cschedCellConfig.m_ulBandwidth<<std::endl;
		//  std::cout<<"RB per flow = "<<rbPerFlow<<std::endl;
		if (rbAllocated + rbPerFlow - 1 > m_cschedCellConfig.m_ulBandwidth)
		{
			// limit to physical resources last resource assignment
			rbPerFlow = m_cschedCellConfig.m_ulBandwidth - rbAllocated;
		...
		uldci.m_rbLen = rbPerFlow;
		bool allocated = false;
		//std::cout << "rnti " << uldci.m_rnti  << "  RB Allocated " << rbAllocated << " rbPerFlow " << rbPerFlow << " flows " << nflows << std::endl;
		NS_LOG_INFO (this << " RB Allocated " << rbAllocated << " rbPerFlow " << rbPerFlow << " flows " << nflows);
		//  std::cout<<" ======== RNTI= "<<(*it).first <<"RB Allocated = " << rbAllocated << " rbPerFlow " << rbPerFlow << " flows " << nflows<<std::endl;

		if ( rbPerFlow==0){
			//std::cout <<"rbPerFlow ZERo="<<rbPerFlow<<std::endl;
			//   std::cout<<"current rnti=" <<it->first<<std::endl;
			//  it++;
			//   std::cout<<"Next rnti="<< it->first<<std::endl;
			//   continue;
		}
		//For a given RNTI iterate over all RB and find its contimuous fitting
		while ((!allocated)&&((rbAllocated + rbPerFlow - 1) < m_cschedCellConfig.m_ulBandwidth) && (rbPerFlow != 0))
		{
			// check availability
			bool free = true;
			...
			if (free)
			{
				...
				rbAllocated += rbPerFlow;
				//	std::cout<<"User= "<<uldci.m_rnti<<" Start PRB= "<<(uint16_t)uldci.m_rbStart<<" Number of rb= "<<rbPerFlow<<std::endl;
				allocated = true;
				break;
			}
			rbAllocated++;
...
PssFfMacScheduler::UpdateDlRlcBufferInfo (uint16_t rnti, uint8_t lcid, uint16_t size)
{
	...
	if (it != m_rlcBufferReq.end ())
	{
		...
		if((*it).second.m_rlcTransmissionQueueSize<size)
			Free_rb=size-(*it).second.m_rlcTransmissionQueueSize;
		else
			Free_rb=0;
		if(Free_rb>=0)
		{
			//std::cout<<"TOTAL SIZE = "<<(*it).second.m_rlcTransmissionQueueSize <<" FREE SIZE = "<<Free_rb<<std::endl;
			//std::cout<<"per rb datasize ="<<per_rb_datasize<<std::endl;
			//std::cout<<"Free Resource block = "<<(Free_rb/per_rb_datasize)<<std::endl;
			Free_rb=(Free_rb/per_rb_datasize);
			double used_rb=50-Free_rb;
			//double total_rb=25;
			//std::cout<<"Load ="<<used_rb/total_rb<<std::endl;
			//double Load =(used_rb/total_rb)*100;
			//std::cout<<"Load of the cell ="<<Load<<" %"<<std::endl;
			//std::cout << "used rbs " << used_rb << std::endl;
			EpcEnbApplication epcenbapp;
			Callback<void,double> cb;
			cb = MakeCallback(&EpcEnbApplication::addusedrbs, &epcenbapp);
			cb(used_rb);
		}
		per_rb_datasize=0;
