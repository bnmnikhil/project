...
{
  NS_LOG_FUNCTION (this);
  Ptr<Node> node=ns3::LteHelper::ltonmap[this];
...
//std::cout<<"in lte-ue-mac "<<E<<std::endl;
...
LteUeMac::DoTransmitPdu (LteMacSapProvider::TransmitPduParameters params)
{
  ...
  Ptr<Node> node=ns3::LteHelper::ltonmap[this];
  Ptr<LiIonEnergySource> E=ns3::LteHelper::ntolimap[node];
  cb=E;//ns3::LteHelper::ntolimap[ns3::LteHelper::ltonmap[this]];
  Callback<void, double> two;
  two = MakeCallback(&LiIonEnergySource::DecreaseRemainingEnergy, cb);
...
//std::cout<<"in lte-ue-mac  node ="<<node<<" obj = "<<E<<std::endl;
 ...
 // std::cout<<"Do transmit PDU"<<std::endl;
}
...
LteUeMac::DoReceivePhyPdu (Ptr<Packet> p)
{
 ...
 two(energyDecreasedRx+energyDecreasedIdle);
  //double energy=cb.GetRemainingEnergy();
//three=MakeCallback(&LiIonEnergySource::GetRemainingEnergy, &cb);
   //std::cout<<" Remaining energy: "<<energy<<std::endl;
