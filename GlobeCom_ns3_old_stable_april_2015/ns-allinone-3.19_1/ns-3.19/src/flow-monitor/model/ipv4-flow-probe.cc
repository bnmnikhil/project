...
Ipv4FlowProbe::SendOutgoingLogger (const Ipv4Header &ipHeader, Ptr<const Packet> ipPayload, uint32_t interface)
{
	...
	if (m_classifier->Classify (ipHeader, ipPayload, &flowId, &packetId))
	{
		...
		//std::cout<<" Transmitted Flow ID = "<<flowId<<" Packet ID = "<<packetId<<" Size =  "<<size<<std::endl;
...
Ipv4FlowProbe::ForwardUpLogger (const Ipv4Header &ipHeader, Ptr<const Packet> ipPayload, uint32_t interface)
{
	...
	if (m_classifier->Classify (ipHeader, ipPayload, &flowId, &packetId))
	{
		...
		if(ipHeader.GetProtocol()==17) {
			...
			cb(flowId,packetId,(double)size,Simulator::Now ().GetSeconds());
		}
		NS_LOG_DEBUG ("ReportLastRx ("<<this<<", "<<flowId<<", "<<packetId<<", "<<size<<");");
		// std::cout<<"Received Flow ID = "<<flowId<<" Packet ID = "<<packetId<<" Size =  "<<size<<std::endl;
