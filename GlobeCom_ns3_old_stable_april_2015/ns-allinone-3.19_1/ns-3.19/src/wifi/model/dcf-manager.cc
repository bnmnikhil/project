...
Time stop=Seconds(0);
  ...
  Time First_node_Tx_time;
  Time busytime=Seconds(0);
  Time previous_duration=Seconds(0);
  int state;
...
DcfManager::ChannelIdleTime(Time now, Time duration){
  ...
  //  std::cout<<"Busy Time = "<<busytime<<std::endl;
   ...
   //std::cout<<"TOTALbusy   "<<1-idletime.GetSeconds()<<std::endl;
    ...
    if(flag1==0) {
    	flag1=1;
    	Simulator::Schedule(Seconds(1.00),&resetidletime);
    }
}
...
void DcfManager::resetidletime() {
	...
	//cb(1-idletime.GetSeconds());
	std::cout<<"Busy time = "<<busytime<<"  Idle time "<<Seconds(1).GetMicroSeconds()-busytime.GetMicroSeconds()<<" Micro seconds"<<std::endl;
