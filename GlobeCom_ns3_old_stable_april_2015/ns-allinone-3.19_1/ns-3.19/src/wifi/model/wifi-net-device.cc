...
//	packet->Print(std::cout);
  ...
  //std::cout<< "Send to" <<realTo <<std::endl;
...
WifiNetDevice::ForwardUp (Ptr<Packet> packet, Mac48Address from, Mac48Address to)
{
 // packet->Print(std::cout);
  ...
  RouterLayer router;
  //std::cout<<"at Wi-Fi netdevice"<<std::endl;
  //packet->Print(std::cout);
// std::cout << router.wifi_Ap_macaddr[0] << "   "<<router.wifi_Ap_macaddr[1]  <<"  "<< from <<"  " << to << std::endl;
size_t number_of_ap= router.wifi_Ap_macaddr.size();
bool valid_ap=false;
for (size_t i=0; i<number_of_ap; i++)
{
	if(router.wifi_Ap_macaddr[i]==from ){
	valid_ap=true;
	}
}
  ...
  if (type != NetDevice::PACKET_OTHERHOST)
    ...
    //  std::cout << "Notify wifinetdevice forwardup" << std::endl;
...
/*
#ifdef BelowPDCP
  if(router.wifi_Ap_macaddr[0]==from) {
    enum NetDevice::PacketType type;
    if (to.IsBroadcast ())
      {
        type = NetDevice::PACKET_BROADCAST;
    ...
    if (!m_promiscRx.IsNull ())
      {
        ...
        m_promiscRx (this, packet, llc.GetType (), from, to, type);
  	   ...
  	   router.forwardtoltepdcp(packet,1);
    }
#endif
*/
}
