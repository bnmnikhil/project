...
YansWifiPhy::StartReceivePacket (Ptr<Packet> packet,
		...
		enum WifiPreamble preamble, Address MacAddr)
{
	NS_LOG_FUNCTION (this << packet << rxPowerDbm << txVector.GetMode()<< preamble);
	////std::cout << "start of startrecvpkt " << MacAddr << std::endl;
	MacAddress=MacAddr;
...
void
YansWifiPhy::EndReceive (Ptr<Packet> packet, Ptr<InterferenceHelper::Event> event,Address MacAddr)
{
	NS_LOG_FUNCTION (this << packet << event);
	NS_ASSERT (IsStateRx ());
	NS_ASSERT (event->GetEndTime () == Simulator::Now ());
	//////std::cout << "start of endrecvpkt " << std::endl;
	//Ipv4Header ipv4Header;
	// packet->PeekHeader(ipv4Header);
	struct InterferenceHelper::SnrPer snrPer;
	snrPer = m_interference.CalculateSnrPer (event);
	m_interference.NotifyRxEnd ();
	Ptr<Packet> temp_pkt=packet->Copy();
	//packet->Print(std::cout);
	//std::cout << std::endl;
	WifiMacHeader wifimacheader;
	temp_pkt->RemoveHeader(wifimacheader);
	/*if(strcmp(wifimacheader.GetTypeString(),"DATA")==0) {
		std::cout << "data " << snrPer.per << std::endl;
	} else {
		std::cout << "other " << snrPer.per << std::endl;
	}*/

		if( wifimacheader.GetAddr1()==Mac48Address::ConvertFrom(MacAddr)) {
			//packet->Print(std::cout);
			//std::cout<<"\n" << Mac48Address::ConvertFrom(MacAddr) << std::endl;
			//std::cout << wifimacheader.GetAddr2() << " snr " << snrPer.snr << " " << snrPer.per<< std::endl;
			/*std::map<double,double> temp_snr_per;
			temp_snr_per[snrPer.snr]=snrPer.per;
			EpcEnbApplication::snr_per[wifimacheader.GetAddr1()]=temp_snr_per;
			EpcEnbApplication::rssi[wifimacheader.GetAddr1()]=event->GetRxPowerW();*/
			EpcEnbApplication epcenbapp;
			Callback<void,double,double,Mac48Address> cb;
			cb = MakeCallback(&EpcEnbApplication::updatewifisinrper, &epcenbapp);
			cb(snrPer.snr,snrPer.per,wifimacheader.GetAddr1());
			// ////std::cout<<"SINR 1 = "<<snrPer.snr<<" Address ="<<wifimacheader.GetAddr1()<<" Duration field = "<<wifimacheader.GetDuration()<<std::endl;

		}

		//  temp_pkt->AddHeader(llcsnapheader);
	//}
	// temp_pkt->AddHeader(wifimacheader);
