...
public:
  LteHelper (void);
 static std::map<Ptr<LteUeMac>,Ptr<Node> > ltonmap;
 static std::map<Ptr<Node>,Ptr<LiIonEnergySource> > ntolimap;
 static std::map<Ptr<LiIonEnergySource>,Ptr<Node> > litonmap; 
 static std::map<Ipv4Address,double > iptoenergymap;
 static std::map<Ptr<Node>,Ipv4Address > ntoipmap;
