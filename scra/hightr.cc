...
{       
	...
	uint16_t numberOfClients=60;//50;//Users
	uint16_t extraues=5;
	double simTime = 10;
	double distance = 0;
	long xvalue=1;
	...
	remoteHostStaticRouting->AddNetworkRouteTo (Ipv4Address ("7.0.0.0"), Ipv4Mask ("255.0.0.0"), 1l);
	...
	integrated_box.Add(enbNodes); /// careful when more than 1 enb is added
	...
	//  }
	mobility.SetPositionAllocator("ns3::RandomDiscPositionAllocator","X",StringValue("0.0"),"Y",StringValue("0.0"),"Rho",StringValue("ns3::UniformRandomVariable[Min=0|Max=10]"),"Theta",StringValue("ns3::UniformRandomVariable[Min=0|Max=360]"));
...
for (uint32_t i=0;i<integrated_ue.Get(0)->GetNDevices();i++){
	...
	RouterLayer::epc_enb_wifi_netdevice[0]=wifiApDevs.Get(0);
	...
	ns3::RouterLayer::wifi_Ap_macaddr[0]=wifiapmacaddress;
  ...
  remHelper->Install ();*/
      ...
      for (uint16_t i = 0; i < numberOfClients; i++)
	{
	   Ipv4Address adr=ns3::LteHelper::ntoipmap[integrated_ue.Get(i)];
            ...
            std::cout <<  integrated_ue.Get(i)<<"   "<<adr<<" "<<battery <<" "<<31752-battery <<std::endl;
