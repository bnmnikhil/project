...
namespace ns3 {
Time stop=MicroSeconds(0);
  Time start_time=MicroSeconds(0);
  Time First_node_Tx_time=MicroSeconds(0);
  Time busytime=MicroSeconds(0);
  Time previous_duration=MicroSeconds(0);
  int state;
  bool collision_counted_for_first_time=true;
  int modified_collision_definition=0;
  //int sem_rx,sem_tx;
  int flag1=0;
/****************************************************************
...
DcfManager::NotifyRxEndOkNow (void)
{
  ...
  /*if(sem_rx==1)
   {
  stop=Simulator::Now();
  //std::cout<<"*********successfull:"<<Simulator::Now()<<std::endl;
  sem_rx=0;
  sem_tx=1;
   }*/
  stop=Simulator::Now();
...
DcfManager::NotifyRxEndErrorNow (void)
{
  ...
  /*if(sem_rx==1)
  {
  //std::cout<<"Unsuccessfull:"<<Simulator::Now()<<std::endl;
  stop=Simulator::Now();
  sem_rx=0;
  sem_tx=1;
  }*/
...
{
  NS_LOG_FUNCTION (this << duration);
// std::cout<<"duration"<<duration.GetMicroSeconds()<<std::endl;

  ChannelIdleTime(Simulator::Now(),duration);
  if (m_rxing)
    {
 ...
 // std::cout<<"SIFS = "<<m_sifs<<std::endl;
  MY_DEBUG ("tx start for " << duration);
  //std::cout<<Simulator::Now().GetMicroSeconds()<<" tx start for = " << duration<<std::endl;
  UpdateBackoff ();
  m_lastTxStart = Simulator::Now ();
  m_lastTxDuration = duration;

/*
  if(state==0) ///0 - reception
  {
      state=1; //1- Changing to tx
      Tx_time=Simulator::Now();
      start_time=Simulator::Now();
      ChannelIdleTime();

      collision_counted_for_first_time= true;
  }
  else if(state==1 ){
      if( Tx_time==Simulator::Now() && collision_counted_for_first_time)
      {
          modified_collision_definition++;
            //std::cout<<"modified_collision_definition  ="<<modified_collision_definition<<std::endl;
            collision_counted_for_first_time=false;
      }
  }*/
  /*if(sem_tx==1)
  {
...
void
DcfManager::ChannelIdleTime(Time now, Time duration){
    //std::cout<<" Current time = "<<start_time.GetMicroSeconds() << " Busytime time = "<<(stop-start_time).GetMicroSeconds()<<std::endl;
    //std::cout<<" Current time = "<<start_time << " Idle time = "<<(start_time-stop)<<std::endl;
	//if the first node has started transmission set the transmission time to simulator now
	//Another node does a transmission and duration of previous transmission < currrent node transmission then it is collision
	if(First_node_Tx_time+previous_duration<=Simulator::Now())
	{
		First_node_Tx_time=Simulator::Now();
		    busytime=busytime+duration;
		    previous_duration=duration;
	}
/*	else if(Simulator::Now()+duration>First_node_Tx_time+previous_duration)
	{

                busytime=busytime+(Simulator::Now()+duration-First_node_Tx_time+previous_duration); 
                 previous_duration=previous_duration + (Simulator::Now()+duration-First_node_Tx_time+previous_duration);
		//std::cout<<"replica"<<std::endl;
	}*/

  //  std::cout<<"Busy Time = "<<busytime<<std::endl;
   // std::cout<<"Channel Idle time"<<idletime<<std::endl;
   //std::cout<<"TOTALbusy   "<<1-idletime.GetSeconds()<<std::endl;

    if(flag1==0) {
    	flag1=1;
    	Simulator::Schedule(Seconds(1.0),&resetidletime);
    }
}
...
double interval=1.0;
void DcfManager::resetidletime() {
	EpcEnbApplication epcenbapp;
	Callback<void,double> cb;
	cb = MakeCallback(&EpcEnbApplication::updatewifiload, &epcenbapp);
	cb(busytime.GetSeconds());
	//std::cout<<"Busy time = "<<busytime<<"  Idle time "<<Seconds(1).GetSeconds()-busytime.GetSeconds()<<" Micro seconds"<<std::endl;
	busytime=Seconds(0.0);

	Simulator::Schedule(Seconds(interval),&resetidletime);
}
